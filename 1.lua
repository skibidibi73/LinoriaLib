...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        -- ==============================================================
        -- 🌟 キーバインドリスト用：トグルチェックボックスを含めたアイテムを作成 🌟
        -- ==============================================================
            ...
            for _, Frame in next, Library.KeybindContainer:GetChildren() do
                if Frame:IsA('Frame') and Frame.Visible then
                    ...
                    if LabelChild and (LabelChild.TextBounds.X + 20 > XSize) then
                        XSize = LabelChild.TextBounds.X + 20 -- 余白20を加算
                    end
                end;
            end;
...
-- ==========================================
-- 📢 Notify カスタマイズ設定
-- メインスクリプトから以下のように上書き可能:
--   Library.NotifyConfig.Prefix    = "[MyScript] "   -- テキスト前に付ける文字列
--   Library.NotifyConfig.DefaultTime = 7             -- デフォルト表示秒数
--   Library.NotifyConfig.AccentColor = Color3.fromRGB(255, 100, 0) -- 左バーの色
--   Library.NotifyConfig.TextColor  = Color3.fromRGB(255, 255, 100) -- テキスト色
--   Library.NotifyConfig.TextSize   = 16             -- フォントサイズ
--   Library.NotifyConfig.OnNotify   = function(text, time) end  -- 通知フック
-- ==========================================
Library.NotifyConfig = {
    Prefix      = '';               -- テキスト前に付ける文字列
    DefaultTime = 5;                -- デフォルト表示秒数
    AccentColor = nil;              -- nilのときLibrary.AccentColorを使用
    TextColor   = nil;              -- nilのときLibrary.FontColorを使用
    TextSize    = 14;               -- フォントサイズ
    OnNotify    = nil;              -- function(text, time) カスタム処理を差し込める
};

function Library:Notify(Text, Time)
    local cfg = Library.NotifyConfig;

    -- カスタムフックが設定されていれば先に呼ぶ
    if type(cfg.OnNotify) == 'function' then
        cfg.OnNotify(Text, Time);
    end;

    -- Prefixを付ける
    local DisplayText = (cfg.Prefix ~= '' and cfg.Prefix or '') .. Text;
    local DisplayTime = Time or cfg.DefaultTime;
    local DisplayTextSize = cfg.TextSize or 14;

    local XSize, YSize = Library:GetTextBounds(DisplayText, Library.Font, DisplayTextSize);
    ...
    local barColor = cfg.AccentColor or Library.AccentColor;
    ...
    -- AccentColorカスタムが設定されていない場合のみレジストリに追加
    if not cfg.AccentColor then
